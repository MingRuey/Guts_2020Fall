 ...
 */
node_t *rev_recursive(node_t **head, node_t *tail)
{
    node_t *next = (*head)->next;
    (*head)->next = tail;
    if (!next) {
    ...
    return rev_recursive(&next, *head);
}
...
```
思路是將 reverse 重新想成一個函示 ```f(linked-list, node)```: 此函式輸入一個串列跟一個節點，將串列反轉，反轉之後將尾端接上輸入的節點，並且回傳新的串列的起頭。如此一來，我們原本的 reverse 就等價於 f(head, NULL)。而 ```f``` 可以被寫成遞迴的形式 :
